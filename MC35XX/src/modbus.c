...
extern unsigned char typeOfDevice;
...
modbusCmd* searchCmd(buf* rx_data)
{
    ...
    case ReadHoldingRegisters:
        if(rx_data->cnt==8)
        {
            ...
            cmd.isCmd = 1;
            cmd.err = 0;
            if(cmd.memAddr<SETTINGS_OFFSET)
            {
                if(cmd.cnt > HoldingRegistersLimit) cmd.err = 0x03;
            }
            else
            {
                if(cmd.cnt > getVarCnt()) cmd.err = 0x03;
            }
            if(cmd.err == 0)
            {
                if(cmd.memAddr<SETTINGS_OFFSET)
                ...
                {
                    if(cmd.memAddr + cmd.cnt > SETTINGS_OFFSET + getVarCnt()) cmd.err = 0x02;
                }
            }
        }
        break;
    ...
    case WriteSingleRegister:
        if(rx_data->cnt==8)
        {
            ...
            {
                if(cmd.memAddr >= SETTINGS_OFFSET + getVarCnt()) cmd.err = 0x02;
            }
        }
        break;
    ...
    case WriteMultipleRegisters:
        ...
        if(rx_data->cnt == 9 + cmd.cnt*2)
        {
            ...
            {
                if(cmd.cnt > SETTINGS_OFFSET + getVarCnt()) cmd.err = 0x03;
            }
            if(cmd.err==0)
            {
                if(cmd.memAddr<SETTINGS_OFFSET)
        ...
        else
        {
            ...
            case ReadHoldingRegisters:
                if(tx_data->cnt >= 5 + cmd->cnt*2)
                {
                    ...
                    for(tmp=0;tmp<cmd->cnt;tmp++)
                    {
                        if(cmd->memAddr + tmp < SETTINGS_OFFSET)
                        {
                        ...
                        {
                            tx_data->ptr[3+tmp*2] = readVar(cmd->memAddr + tmp - SETTINGS_OFFSET) >> 8;
                            tx_data->ptr[4+tmp*2] = readVar(cmd->memAddr + tmp - SETTINGS_OFFSET) & 0xFF;
                        }
                    }
                    tmp=GetCRC16(tx_data->ptr,3+cmd->cnt*2);
                    tx_data->ptr[3+cmd->cnt*2]=tmp>>8;
            ...
            case WriteSingleRegister:
                if(tx_data->cnt >= 8)
                {
                    ...
                    tx_data->cnt = 8;
                    write_data(tx_data);
                    if(cmd->memAddr< SETTINGS_OFFSET) holdReg[cmd->memAddr] = cmd->cnt;
                    else
                    {
                        writeVar(cmd->memAddr - SETTINGS_OFFSET,cmd->cnt);
                    }
                }
                break;
            case WriteMultipleCoils:
            ...
            case WriteMultipleRegisters:
                if(tx_data->cnt >= 8)
                {
                    ...
                    for(tmp=0;tmp<cmd->cnt;tmp++)
                    {
                        ...
                        {
                            writeVar(cmd->memAddr + tmp - SETTINGS_OFFSET, (((unsigned short)cmd->ptr[tmp*2])<<8) | cmd->ptr[tmp*2+1]);
                        }
                    }
                }
                break;
            case ReportSlaveID:
                if(tx_data->cnt >= 11)
                {
                    ...
                    // additional data
                    tx_data->ptr[5] = 15;   // code word high byte
                    tx_data->ptr[6] = 10;   // code word low byte
                    tx_data->ptr[7] = 0x01; // version high byte
